...
contract("CarSharing", function (accounts) {
  const [contractOwner, alice, ben] = accounts;
  const deposit = web3.utils.toBN(2);

  beforeEach(async () => {
    instance = await CarSharing.new();
  });
  ...
  // unit test to check if contract is owned by contract owner
  ...
  // unit test to check if unregistered user is able to register
  it("should return registered as true", async () => {
    var result = await instance.registerDriver("alice", "S9132313E", { from: alice });
    const accountAddress = result.logs[0].args.user;
  ...
  // unit test to check if unregistered user are marked as registered
  it("should not return unregistered as registered", async () => {

    const benRegistered = await instance.getRegistrationStatus(ben, { from: ben });
    assert.equal(
      benRegistered,
      false,
      "unregistered user is returned as registered",
    );
  });

  // unit test to check if registered user are able to reserve car successfully
  it("should return user successfully reserve car", async () => {
    const stakeAmount = 2;
    await instance.registerDriver("alice", "S9132313E", { from: alice });
    var plateNo = "SJW1234E";
    const result = await instance.rentCar(stakeAmount, plateNo, { from: alice });
    const accountAddress = result.logs[0].args.user;
    assert.equal(
      accountAddress,
      alice,
      "unable to rent car, check if user is able to register successfully",
    );
  });

  // unit test to check if registered user are able to release reserved car successfully
  it("should return user successfully release car reservation", async () => {
    const stakeAmount = 2;
    var plateNo = "SJW1234E";
    //const result = 0;
    await instance.registerDriver("alice", "S9132313E", { from: alice });

    await instance.rentCar(stakeAmount, plateNo, { from: alice });
    const result = await instance.unrentCar(plateNo, { from: alice });
    const accountAddress = result.logs[0].args.user;
    assert.equal(
      accountAddress,
      alice,
      "unable to unrent car, check for error",
    );
  });

  // it("should refund correct amount", async () => {
  //   const initialAmount = 0;
  //   const stakeAmount = 2;
  //   await instance.registerDriver("alice", "S9132313E", { from: alice });
  //   await instance.rentCar({ from: alice, value: stakeAmount });
  //   await instance.unrentCar({ from: alice });
  //   const balance = await instance.getBalance.call({ from: alice });

  //   assert.equal(
  //     balance.toString(),
  //     initialAmount.toString(),
  //     "balance incorrect after refund, check refund method",
  //   );
  // });
